#!/usr/bin/env bash
# OTEL ingest benchmark: cold (schema discovery) then warm (same DuckDB process).
#
# Each session is one DuckDB process:
#   cold ingest + CHECKPOINT  → timed
#   DELETE FROM table         → untimed (keep evolved DDL)
#   warm ingest + CHECKPOINT  → timed (fresh timestamps, columns_added must be 0)
#
# Examples:
...
usage: $(basename "$0") [options]
  ...
  --quick           100k records, 1 run (CI / remote smoke)
...
mkdir -p "${DATA}" "${WORK}" "${RESULTS}"
chmod +x "${SESSION}" 2>/dev/null || true
...
for sig in "${SIGNALS[@]}"; do
	...
	for (( r = 1; r <= RUNS; r++ )); do
		...
		while IFS= read -r line; do
			...
			line_i=$((line_i + 1))
		done < <("${PYTHON}" "${SESSION}" "${DUCKDB}" "${EXT}" "${db}" "${table}" \
			"${cold_path}" "${warm_path}" "${transform}")
		if [[ -z "${cold_line}" || -z "${warm_line}" ]]; then
			echo "session produced fewer than 2 result lines for ${sig} run ${r}" >&2
			exit 1
		fi
		if (( ! WARM_ONLY )); then
...

{
	...
	if (( ${#JSON_PARTS[@]} > 0 )); then
		printf "    %s" "${JSON_PARTS[0]}"
		local_i=1
		while (( local_i < ${#JSON_PARTS[@]} )); do
			printf ",\n    %s" "${JSON_PARTS[$local_i]}"
			local_i=$((local_i + 1))
		done
		printf "\n"
	fi
